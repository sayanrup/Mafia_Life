...
function tryAction(fn) {
  if (GAME.player.actionsRemaining <= 0) {
    showMsg('No Actions Left', 'You\'ve used all your actions this turn. End your turn to continue.');
    return;
  }
  const res = fn();
  if (res && res.ok === false) {
...
function endTurn() {
  ...
  state.meta.day++;
  state.meta.turn++;
  state.player.actionsRemaining = ACTIONS_PER_TURN;
...
function continueAsFamilyMember(memberId) {
  ...
  state.player = {
    ...
    crew: { size: 1, quality: 1, loyalty: 50, weaponTier: 0, upkeepPaid: true, autoPayUpkeep: true },
